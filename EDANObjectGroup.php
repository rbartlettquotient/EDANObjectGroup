  ...
  namespace EDAN\OGMT {
    ...
    class ObjectGroups extends \EDAN\EDANBase {
      ...
      public function load($withChildren = false, $start = NULL, $rows = NULL, $sort = NULL, $sortDir = NULL,
        ...
        // deleted gets overridden by published
        // if both deleted and published are set, we used the value of published
        if(NULL !== $published) {
        ...
        if(false == $adminView) {
          $service = 'ogmt/v1.0/ogmt/objectgroups.htm';
        }
    ...
    class ObjectGroup extends \EDAN\EDANBase {
      ...
      public function load($objectGroupId, $adminView = true) {
        ...
        $this->objectGroupId = $objectGroupId;
        $service = 'ogmt/v1.0/adminogmt/objectgroup.htm';
        if(false == $adminView) {
          $service = 'ogmt/v1.0/ogmt/objectgroup.htm';
        }
        $params = array(
      ...
      public function save() {
        ...
        $publishedValue = (true == $this->published) ? 1 : 0;
      ...
      public function markForDeletion() {
        ...
        $original_published = $this->published;
        ...
        if(!$ok) {
          $this->deleted = false;
          $this->published = $original_published;
        }
        return $ok;
      ...
      public function setPageOrder() {
        ...
        /*
        if(!is_array($pageIds)) {
          $this->errors[] = "Cannot set page order. Parameter pageIds must be array. setPageOrder()";
          return false;
        }
        */
        ...
        /*
        // re-order this object group's pages based on the supplied pageIds
        $newPageArray = array();
        foreach($pageIds as $key => $pageId) {
          foreach($this->objectGroupPages as $oldkey => $thisPage) {
            if($thisPage->pageId == $pageId) {
              // add to our new order/array
              $newPageArray[] = $thisPage;
            }
          }
        }

        // make sure all of our existing pages are in the new array
        //@todo - do we need to do this? What happens if we pass back page orders that don't represent all pages?
        foreach($this->objectGroupPages as $oldkey => $thisPage) {
          $bFoundPage = false;
          foreach($newPageArray as $newkey => $newPage) {
            if($thisPage->pageId == $newPage->pageId) {
              $bFoundPage = true;
            }
          }
          if(!$bFoundPage) {
        ...
        $this->objectGroupPages = $newPageArray;
        */
    ...
    class ObjectGroupPage extends \EDAN\EDANBase {
      ...
      public $title;
      public $listTitle;
      public $feature; // feature currently has no defined purpose
      public $content;
      public $pageImageUri;
      public $uri;
      public $settings;
    ...
    class ObjectList extends \EDAN\EDANBase {
      ...
      // if $items is not set, this object listing's items will be used
        ...
        /*
         * this is the JSON encoding for params sent by the existing ogmt modules to the api
        {"objectGroupId":"dpt-1451339401559-1451343216472-0","action":"add","item":"nmnhbotany_2022118","pageId":"dpt-1451417291281-1451490155174-0"}
         */
